...
/* Professor password for the hub role's connect_cb (fleet/estop is the one
 * thing the open ACL can't hand out for free). Stored in NVS, NOT compiled in:
 * PROFESSOR_PASS is a plaintext literal in the image, `robot` is a public repo,
 ...
 * PROFESSOR_PASS default, so an un-provisioned board behaves as before.
...
void rover_config_load_professor_pass(char pass[65]);
